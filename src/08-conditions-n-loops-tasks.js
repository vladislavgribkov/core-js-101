...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  array.push(bracketStart);
  array.push(bracketEnd);
  if (a > b) {
    array.splice(1, 0, ...[b, ', ', a]);
  } else {
...
function getDigitalRoot(num) {
  ...
  const result = array.reduce((prev, next) => Number(prev) + Number(next), 0);
  return num > 10 ? getDigitalRoot(result) : result;
}
...
function getMatrixProduct(m1, m2) {
  const result = [];
  m1.forEach((element, i) => {
    result[i] = [];
    for (let j = 0; j < m2[0].length; j += 1) {
      result[i][j] = element.reduce((prev, next, k) => prev + next * m2[k][j], 0);
    }
  });
  return result;
}
